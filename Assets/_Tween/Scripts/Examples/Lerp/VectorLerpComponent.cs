...
namespace Tween.Examples
{
    ...
    internal class VectorLerpComponent : MonoBehaviour
    {
        ...
        private float ConvertRatio(float ratio, RatioConversion conversion) {
            switch (conversion)
            {
                case RatioConversion.Linear:
                    return ratio;
                    case RatioConversion.Quad: 
                    return Mathf.Pow(ratio, 2);
                case RatioConversion.Root:
                    return Mathf.Sqrt(ratio);
                default:
                    throw new ArgumentOutOfRangeException(nameof(RatioConversion));
            }; 
        }
    }
}
